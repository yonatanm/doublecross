...
import { clueHtmlToPrintHtml } from "@/lib/clue-format"
...
export function openPrintWindow(crossword: Crossword, options: PrintOptions = {}) {
  ...
  const renderClueItems = (clues: typeof clues_across) =>
    clues.map((c) => `<div class="clue"><b>${c.number}.</b> ${clueHtmlToPrintHtml(c.clue)} <span dir="ltr" style="white-space:nowrap">${escapeHtml(c.answerLength.replace(/,\s*/g, ","))}</span></div>`).join("")
