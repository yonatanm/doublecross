...
- **Editor proposals**: flat proposal gallery — each "שבץ מילים" click generates K proposals sorted by score, replacing any previous batch. Proposals shown as a thumbnail strip (mini grids, cellSize=6) with prev/next arrows. Each proposal has its own `highlightedCells`. Loading from Firestore creates a single proposal
