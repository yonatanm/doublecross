...
- **Print** (`src/lib/print-crossword.ts`): opens a new window with standalone HTML that auto-triggers `window.print()`. Two modes: **regular** (grid + clues on same page) and **separate/glasses** (`separateClues` or `largeFont` — grid fills page 1, clues on page 2 with larger fonts). Highlighted cells show their letter on white background; other cells are empty. Uses `print-color-adjust: exact` to preserve black backgrounds. Cell size is computed dynamically to fit A4 page (263mm usable height accounting for browser chrome). Clues use a 2-column CSS grid (`1fr 1fr`) — מאוזן on the right, מאונך on the left — each in its own column div, so sections never mix and overflow naturally to the next print page
